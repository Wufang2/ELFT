...
class ELGFAttention(nn.Module):
    def __init__(self, channel, kernel_size=7):
        super(ELGFAttention, self).__init__()
        pad = kernel_size // 2
        self.conv = nn.Conv1d(channel, channel, kernel_size=kernel_size, padding=pad, groups=channel, bias=False)
    ...
    def init_weights(self):
        ...
        init.kaiming_normal_(self.conv.weight, mode='fan_out', nonlinearity='sigmoid')
        # Initialize group normalization weights
    ...
    def __init__(self, name: str,
        ...
        for idx, layer_index in enumerate(return_interm_indices):
            for i, layer in enumerate(backbone.layer4):
                if i == layer_index:
                    # Replace the 3x3 convolutional layer in the Bottleneck module with ELGFA.
                    layer.conv2 = ELGFAttention(layer.conv2.out_channels)
