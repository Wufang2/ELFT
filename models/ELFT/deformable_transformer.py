        ...
        if encoder_layer_last is not None:
            if num_expansion == 1:
                ...
                for i in range(num_expansion):
                    ...
                    # The final high-resolution feature map utilizes low-level features.
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if not dropflag:
                ...
                if (layer_id + 1) % (self.num_layers / self.num_expansion) == 0:
                    ...
                    # the next block, tgt remains at a high level.
                    tgt = output[:, level_start_index[4 - self.enc_scale]:]
                    src = output
                else:
                    # Here, tgt only contains tokens from high-level feature maps.
                    output = layer(src=src, tgt=tgt, pos=pos_tgt, reference_points=reference_points_tgt,
                                   spatial_shapes=spatial_shapes, level_start_index=level_start_index,
                                   key_padding_mask=key_padding_mask)
                    tgt = output
                    # The output of the high-level feature maps is updated into src, replacing the latter part of src.
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    def forward_ffn(self, src):
        ...
        cb = 0.5 * src2 + 0.5 * src2.mean(dim=1, keepdim=True)
        src = src + cb
        src = self.norm2(src)
        return src
    ...
    def forward(self, src, tgt, pos, reference_points, spatial_shapes, level_start_index, key_padding_mask=None):
        ...
        tgt2 = self.self_attn(self.with_pos_embed(tgt, pos), reference_points, src, spatial_shapes, level_start_index,
                              key_padding_mask)
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward_ffn(self, tgt):
        ...
        cb = 0.5 * tgt2 + 0.5 * tgt2.mean(dim=1, keepdim=True)
        tgt = tgt + cb
        tgt = self.norm3(tgt)
        return tgt
